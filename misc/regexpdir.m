function dirlist = regexpdir(rootdir, expstr, options)
...
%    REGEXP. 
% 
% options by jojo added:
% option.recursive = true / false
% option.absoluteFilepath = true / false
% 
...
if exist('options','var')
    ...
    if isfield(options,'absoluteFilepath')
        returnAbsolutePath = options.absoluteFilepath;
    end
end
%jojo: old version: if ~exist('recursive','var'); recursive = true; end
...
else % relative filename saving
    for i = find(~[dirtree.isdir]);
        if regexpi(strrep(fullfile(rootdir, dirtree(i).name), basedir, ''), expstr);
            dirlist = [dirlist; dirtree(i).name];
        end
    end
end
